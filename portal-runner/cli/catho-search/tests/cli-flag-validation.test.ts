...
describe("Catho CLI flag validation", () => {
  ...
  test("search without a keyword is refused", async () => {
    expectError(await runCLI(["search", "-l", "Curitiba"]), "NO_QUERY");
  });
  ...
  test("--browser-headers is an accepted flag on both commands", async () => {
    // It must not trip the unknown-flag guard. Validation still fails first on
    // the missing keyword, which is what keeps this test network-free.
    expectError(await runCLI(["search", "--browser-headers"]), "NO_QUERY");
    expectError(await runCLI(["detail", "--browser-headers"]), "NO_ID");
  });
});
