...
// Every case here fails validation before any network request, so this file is
...
describe("Gupy CLI flag validation", () => {
  test("an unknown flag is rejected instead of silently dropped", async () => {
    expectError(await runCLI(["search", "-q", "dados", "--cidade", "Curitiba"]), "UNKNOWN_FLAG");
  });

  test("a search with no filter at all is refused", async () => {
  ...
  test("--remote only accepts the shared vocabulary", async () => {
    expectError(await runCLI(["search", "-q", "dados", "--remote", "talvez"]), "BAD_ARG");
  });
